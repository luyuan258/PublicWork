...
class GeminiClientImpl {
        ...
        try {
            ...
            if (!isGeminiResponse(rawResponse)) {
                ...
                if (asRecord['error']) {
                    ...
                    return Err(new GeminiAPIError('Gemini API returned an error response', {
                        provider: 'gemini',
                        stage: 'api_error',
                        upstreamMessage: typeof error['message'] === 'string' ? error['message'] : 'Unknown error',
    ...
    handleError(error, prompt) {
        ...
        if (isNetworkError(error)) {
            return Err(new NetworkError('Network error during Gemini image generation', 'Check your internet connection and try again', error instanceof Error ? error : undefined));
        }
        // Check if it's an API-specific error
        if (this.isAPIError(error)) {
            return Err(new GeminiAPIError('Failed to generate image with Gemini', {
                provider: 'gemini',
                prompt,
