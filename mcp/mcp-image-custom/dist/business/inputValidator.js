...
export function validateImagePath(imagePath) {
    ...
    if (!SUPPORTED_EXTENSIONS.includes(ext)) {
        return Err(new InputValidationError(`Unsupported image format: ${ext}. Supported formats: ${SUPPORTED_EXTENSIONS.join(', ')}`, `Please provide an image with one of these extensions: ${SUPPORTED_EXTENSIONS.join(', ')}`));
    }
    return Ok(imagePath);
}
/**
